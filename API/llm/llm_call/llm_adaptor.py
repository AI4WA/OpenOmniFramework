...
class LLMAdaptor:
    ...
    def get_llm_model_config(self):
        ...
        model_config = LLMConfigRecords.objects.filter(model_name=self.model_name).first()
        if model_config is None:
            raise ValueError(f"Model {self.model_name} is not supported")

        model_path = Path(
            settings.BASE_DIR / "llm" / "llm_call" / "models" / model_config.model_family / model_config.filename)
    ...
    def create_completion(self, prompt: str):
        ...
        if model_config.model_type == MT_LLAMA:
            model_path = Path(
                settings.BASE_DIR / "llm" / "llm_call" / "models" / model_config.model_family / model_config.filename)
            llm = Llama(
                model_path=model_path.as_posix(), embedding=False
            )
    ...
    def create_chat_completion(self, prompt: str):
        ...
        if model_config.model_type == MT_CHATGLM:
            model_path = Path(
                settings.BASE_DIR / "llm" / "llm_call" / "models" / model_config.model_family / model_config.filename)
            chatglm_pipeline = chatglm_cpp.Pipeline(model_path=model_path.as_posix())
            output = chatglm_pipeline.chat([chatglm_cpp.ChatMessage(role="user", content=prompt)])
            logger.critical(f"Response: {output}")
            return {
                "role": output.role,
    ...
    def create_embedding(self, text: str):
        ...
        if model_config.model_type == MT_LLAMA:
            model_path = Path(
                settings.BASE_DIR / "llm" / "llm_call" / "models" / model_config.model_family / model_config.filename)
            llm = Llama(
                model_path=model_path.as_posix(), embedding=True
            )
            return llm.create_embedding(text)
        raise ValueError(f"Model {model_config.model_type} is not supported for embedding")
