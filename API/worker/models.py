...
class Task(models.Model):
    ...
    work_type = models.CharField(max_length=100, choices=[('llm', 'LLM'), ('stt', 'Speech2Text')],
                                 help_text='Select the type of work')
    parameters = models.JSONField(default=dict, blank=True, null=True, help_text='Enter the parameters for the task')
    ...
    def create_task(cls, user, name, work_type, parameters, description=""):
        task = cls(user=user, name=name, work_type=work_type, parameters=parameters, description=description)
        task.save()
        return task
