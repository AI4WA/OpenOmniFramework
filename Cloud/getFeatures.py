...
class getFeatures():
    ...
    def getTextEmbedding(self, input_dir, text_index):
        text_path = os.path.join(self.working_dir, input_dir, f"{text_index}.txt")
        if not os.path.exists(text_path):
            print("can not find:", text_path)
            return 0
        text = open(text_path, encoding = 'utf-8').read()  #需要utf-8格式
    ...
    def getAudioEmbedding(self, input_dir, text_index):
        audio_pathes = glob(os.path.join(self.working_dir, input_dir, f"audio{text_index}/*.wav"))
        features = []
        tem_feature=0
        for audio_path in tqdm(audio_pathes):   #对一个文件夹中的所有音频取平均
        ...
        return feature  # (seq_len, 33)

    def getVideoEmbedding(self, input_dir, index, pool_size=5):
        csv_path = f'{input_dir}/features{index}/images{index}.csv'
        if not os.path.exists(csv_path):
            print("can not find:", csv_path)
            return 0
        ...
        #         # 对齐并裁剪人脸，然后将人脸传递给resnet模型提取特征
        #         face = mtcnn(img)
        #         face = face.unsqueeze(0)
        #         embeddings = resnet(face) #512
        #
        #         # embeddings现在是一个张量，其中包含每个检测到的人脸的特征向量
        #         face_features.append(embeddings)
        #     else:
        #         print('not check the face')
        #         1/0
    ...
    def handleImages(self, index):
        image_dirs = sorted(glob(os.path.join(self.working_dir, 'frames', f'images{index}')))
        for image_dir in tqdm(image_dirs):
            output_dir = os.path.join('frames', f'features{index}')
    ...
    def results(self, output_dir):
        ...
        # video
        csv_path = os.path.join(self.data_dir, 'video/OpenFace', video_id, clip_id, clip_id + '.csv')
...
def parse_args():
    ...
    parser.add_argument('--data_dir', type=str, default='/home/lyy/code/MMSA-master/',
                        help='path to CH-SIMS')
    parser.add_argument('--openface2Path', type=str, default="/home/lyy/OpenFace-master/build/bin/FeatureExtraction",
                        help='path to FeatureExtraction tool in openface2')
    parser.add_argument('--pretrainedBertPath', type=str, default='/home/lyy/code/MMSA-master/chinese_Bert/',
